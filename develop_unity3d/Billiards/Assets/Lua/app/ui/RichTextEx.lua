...
一个简单的富文本 Label，用法
	...
	local txt = RichTextEx:create() -- 或 RichTextEx:create(26, nb.c3b(10, 10, 10))
...
function _M:ctor(fontSize, textColor)
	...
	self._textColorDef	= textColor or nb.c3b(11, 11, 11)
...
function _M.defaultCb(text, sender)
	...
	if str_sub(text, 1, #BLINK) == BLINK then
		...
		lbl:runAction(nb.RepeatForever:create(nb.Blink:create(10, 10)))
		return lbl
	elseif str_sub(text, 1, #ROTATE) == ROTATE then
		...
		lbl:setTextColor(c3b_to_c4b(sender._textColor))
		lbl:runAction(nb.RepeatForever:create(nb.RotateBy:create(0.1, 5)))
		return lbl
	elseif str_sub(text, 1, #SCALE) == SCALE then
		local lbl = ccui.Text:create(self:htmlDecode(str_sub(text, #SCALE + 1)), "", sender._fontSize)
		lbl:setTextColor(c3b_to_c4b(sender._textColor))
		lbl:runAction(nb.RepeatForever:create(nb.Sequence:create(nb.ScaleTo:create(1.0, 0.1), nb.ScaleTo:create(1.0, 1.0))))
		return lbl
	end
...
function _M.defaultImgCb(text)
	...
	local spf, img = nb.SpriteFrameCache:getInstance():getSpriteFrame(text), nil
	if spf then
	--		img = nb.Sprite:createWithSpriteFrame(spf)
		img = ccui.ImageView:create(text, ccui.TextureResType.plistType)
	elseif nb.FileUtils:getInstance():isFileExist(text) then
	--	  	img = nb.Sprite:create(text)
		img = ccui.ImageView:create(text, ccui.TextureResType.localType)
	end

	if img and w and h and w > 0 and h > 0 then
		img:ignoreContentAdaptWithSize(false) -- nb.Sprite can't do this, so we use ccui.ImageView
		img:setContentSize(nb.size(w, h))
	end
...
function _M:addCustomNode(node)
	if node then
		...
		if anc.x ~= 0.0 or anc.y ~= 0.0 then
			...
			node:addChild(tmp)
			tmp:setPosition(nb.p(siz.width * anc.x, siz.height * anc.y))
		end
		local obj = ccui.RichElementCustomNode:create(0, nb.c3b(255,255,255), 255, node)
...
function _M:setText(text, callback)
	...
	while i <= len do
		...
		elseif c == C_LN or c == C_TAB then
			...
			obj = nb.Node:create()
			if c == C_LN then
				obj:setContentSize(nb.size(self:getContentSize().width, 1))
			else
				obj:setContentSize(nb.size(self._fontSize * 2, 1))
			end
