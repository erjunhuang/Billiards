pcall(require,"pack")
local utils = nb.load("utils")
local net = nb.load("net")
local PacketPaser = import(".PacketParser")
...
function Socket:readInt(packetId, defaultValue)
    ...
    local n
    packet.position,n = string.unpack(packet.data,self.m_endian .. 'i', packet.position)
    return n
end
...
function Socket:readInt64(packetId, defaultValue)
    ...
    local n
    packet.position,n = string.unpack(packet.data,self.m_endian .. 'g' ,packet.position)
    return n
end
...
function Socket:readByte(packetId, defaultValue)
    ...
    local n
    packet.position,n = string.unpack( packet.data, self.m_endian .. 'b',packet.position)
    return n
end
...
function Socket:readString(packetId)
    local packet = self.__packets[packetId]
    local len = self:readUInt(packetId, 0)
    local str
    
    if len == 0 then
        packet.position,str = packet.position,""
    elseif len == 1 then
        packet.position,str = string.unpack( packet.data, self.m_endian .. 'A' .. tostring(len),packet.position)
    else
        packet.position,str = string.unpack( packet.data,self.m_endian .. 'A' .. tostring(len-1), packet.position)
...
function Socket:readShort(packetId, defaultValue)
    ...
    local n
    packet.position,n = string.unpack( packet.data,self.m_endian .. 'h', packet.position)
    return n
end
...
function Socket:writeInt(packetId,val)
    local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'i', val))
end
...
function Socket:writeInt64(packetId,val)
    local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'g', val))
end

function Socket:writeUInt64(packetId,val)
    local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'G', val))
end
...
function Socket:writeByte(packetId,val)
    local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'b', val))
end
...
function Socket:writeShort(packetId,val)
    local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'h', val))
end
...
function Socket:writeString(packetId,val)
    self:writeUInt(packetId,#val + 1)
     local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'A', val))
    self:writeByte(packetId,0)

end

function Socket:writeBinary(packetId,val)
    self:writeUInt(packetId,#val)
    local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'A', val))
end
...
function Socket:writeLong(packetId,val)
    local packet = self.__packets[packetId]
    packet:write(string.pack(self.m_endian .. 'l', val))
end
