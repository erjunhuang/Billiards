...
function GameController:createNodes( ... )
    ...
    self.animManager:createNodes(self)
...
function GameController:onUserReconnect(pack)
    if not pack then
        return
    end
    local model = self.model
    model:onUserReconnect(pack)

    if (type(pack.ballsInfo) ~= "table" and #pack.ballsInfo <= 0) then
        --未开过球，无球位置信息
        self.tableManager:setupLuaBallsPosition()
    else
        --开过球
        self.tableManager:setupLuaBallsPositionByBallsInfo(model.__lastBallsResult)
    end
...
function GameController:onBroadUserTurn(pack)
    dump(pack,"GameController:onBroadUserTurn")
    local model = self.model
...
function GameController:onBroadUserLogout(pack)
    local model = self.model
     local player = model:onBroadUserLogout(pack)
     self.seatManager:removePlayer(player)
end


function GameController:onSelfLoginOut(pack)
...
function GameController:onBroadUserLogin(pack)
    ...
    dump(pack,"GameController:onBroadUserLogin")
     local model = self.model
