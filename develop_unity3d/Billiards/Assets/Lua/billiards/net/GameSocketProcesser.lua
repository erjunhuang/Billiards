...
function GameSocketProcesser:onBroadUserLogout(packetInfo)
	local info = {}
	info.userId = packetInfo.uid
	
	self.__controller:onBroadUserLogout(info)
end
...
function GameSocketProcesser:onUserReconnect(packetInfo)
	...
	info.gameStatus = packetInfo.gameStatus --游戏状态 0-游戏未开始 1-确定花色 2-正常击球流程
	info.ballsInfo = packetInfo.balls --BallRes

	info.ballColor = packetInfo.color --目标花色

	dump(info,"超神重连",4)
	self.__controller:onUserReconnect(info)
end
...
function GameSocketProcesser:onSelfLoginOut(packetInfo)
	self.__controller:onSelfLoginOut(packetInfo)
end
...
GameSocketProcesser.s_severCmdEventFuncMap = 
{
	...
	[GameSocketCmd.BROADCAST_USER_LOGOUT_GAME]		= GameSocketProcesser.onBroadUserLogout,
	[GameSocketCmd.LOGOUT_GAME] = GameSocketProcesser.onSelfLoginOut,
	[GameSocketCmd.CMD_C2GAMESER_RECONNECT] = GameSocketProcesser.onUserReconnect,
