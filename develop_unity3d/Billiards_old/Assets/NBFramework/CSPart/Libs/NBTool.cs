...
namespace NBFramework
{
    ...
    public class NBTool
    {
        ...
        {
            var rangeArr = NBTool.Split<float>(str, '~', '-');
        ...
        */
        ...
        public static void Array2Table(uint[] arr)
        {
            //Log.Info("Array2Table000000000");
            //LuaTable bb = new LuaTable();
            //for (int i = 0; i < arr.Length; i++)
            //{
            //  //  Log.Error("Array2Table", arr[i]);
            //    bb.Set<Int32, UInt32>(i, arr[i]);
            //}

        }
        public static uint[] Table2Array(LuaTable bb)
        {
            //int length = bb.Length;
            //ArrayList list1 = new ArrayList();
            //for (int i = 1; i <= length; i++)
            //{
            //    list1.Add(bb.Get<Int32, UInt32>((i)));
            //}

            //return (uint[])list1.ToArray(typeof(UInt32));

            int length = bb.Length;
            var arr = new uint[length];
            for (int i = 1; i <= length; i++)
            {
                arr[i - 1] = bb.Get<Int32, UInt32>(i);
            }
            return arr;
        }
